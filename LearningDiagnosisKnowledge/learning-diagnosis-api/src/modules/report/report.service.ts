...
export class ReportService {
  ...
  async generateReport(
    ...
    report.creatorId = Number(currentUser.id);
  ...
  async deleteReport(id: number, currentUser: RequestUser): Promise<void> {
    ...
    if (currentUser.role !== UserRole.ADMIN && Number(report.creatorId) !== Number(currentUser.id)) {
  ...
  private applyPermissionFilter(queryBuilder: any, currentUser: RequestUser) {
    ...
    if (currentUser.role === UserRole.STUDENT) {
      queryBuilder.andWhere('report.studentId = :studentId', { studentId: Number(currentUser.id) });
    }
  ...
  private async checkReportPermission(report: LearningReport, currentUser: RequestUser) {
    ...
    if (currentUser.role === UserRole.STUDENT && Number(report.studentId) === Number(currentUser.id)) {
      return true;
    }

    if (currentUser.role === UserRole.TEACHER) {
      if (Number(report.creatorId) === Number(currentUser.id)) {
        return true;
      }
