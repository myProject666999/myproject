...
func main() {
	log.Println("Fixing database table structure...")
	...
	_, err = rootDB.Exec("DROP DATABASE IF EXISTS " + config.AppConfig.DBName)
	if err != nil {
		log.Fatalf("Failed to drop database: %v", err)
	}

	_, err = rootDB.Exec(fmt.Sprintf("CREATE DATABASE %s DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci", config.AppConfig.DBName))
	if err != nil {
	...
	insertSettingsSQL := `INSERT INTO settings (key_name, value, description) VALUES
	...
	_, err = db.Exec(insertSettingsSQL)
	if err != nil {
		log.Fatalf("Failed to insert settings: %v", err)
	}

	log.Println("Database table structure fixed successfully!")
}
