...
function NewsManage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res = await request.get(`/admin/news?page=${page}&page_size=${pageSize}`);
  ...
  const handleSubmit = async (values) => {
    try {
      if (editingItem) {
        await request.put(`/admin/news/${editingItem.id}`, values);
        message.success('更新成功');
      } else {
        await request.post('/admin/news', values);
