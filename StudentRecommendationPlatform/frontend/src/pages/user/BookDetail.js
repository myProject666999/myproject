...
import { ArrowLeftOutlined, EyeOutlined, StarOutlined, StarFilled, LikeOutlined } from '@ant-design/icons';
...
function BookDetail() {
  ...
  const checkFavorite = async () => {
    ...
    try {
      const res = await request.get('/user/favorites?type=book');
      const favorites = res.data || [];
      const bookId = parseInt(id);
      const found = favorites.find(f => f.target_id === bookId);
      setIsFavorite(!!found);
  ...
  const handleFavorite = async () => {
    ...
    try {
      if (isFavorite && favoriteId) {
        ...
        const res = await request.post('/user/favorites', { type: 'book', target_id: parseInt(id) });
        setIsFavorite(true);
