...
import { Card, Table, Button, Form, Input, Select, Modal, message, Popconfirm, Space } from 'antd'
...
const Nurses = () => {
  ...
  const fetchData = async (page = 1, pageSize = 10, params = {}) => {
    setLoading(true)
    try {
      const res = await request.get('/nurses', {
        params: {
          page,
          page_size: pageSize,
          ...params,
  ...
  const fetchUsers = async () => {
    try {
      const res = await request.get('/users', {
        params: { role: 'nurse', page_size: 100 },
      })
  ...
  const handleDelete = async (id) => {
    try {
      await request.delete(`/nurses/${id}`)
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingId) {
        await request.put(`/nurses/${editingId}`, values)
        message.success('更新成功')
      } else {
        await request.post('/nurses', values)
      ...
      console.error('Submit nurse error:', error)
    }
  }
  ...
  return (
    <div>
      <div className="page-header">
        <h2 className="page-title">护士管理</h2>
